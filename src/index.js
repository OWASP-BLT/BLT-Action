...
// I added(vansh malik) : bounty constants + helpers
const BOUNTY_COMMENT_MARKER = "<!-- blt-bounty -->";
const BOUNTY_LABEL_COLOR = "0e8a16";
...
function parseBountyAmount(raw) {
  // /bounty $5  | /bounty 5 | /bounty $10.50
  const m = raw.trim().match(/^\/bounty\s+\$?\s*(\d+(?:\.\d{1,2})?)\s*$/i);
  if (!m) return null;
  const v = Number(m[1]);
...
function getBountyInfoFromLabels(labels) {
  let total = 0;
  const bountyLabels = [];
  ...
  for (const l of labels || []) {
    ...
    const mm = t.match(/^\$(\d+(?:\.\d{1,2})?)\s+Bounty$/i);
    if (mm) {
      bountyLabels.push(t);
      total = Number(mm[1]) || total;
    }
  }

  return { total, bountyLabels };
}

async function ensureLabelExists(octokit, owner, repoName, labelName) {
  try {
    ...
    if (e.status !== 404 && e.statusCode !== 404) return;
  }
  ...
  try {
    await octokit.rest.issues.createLabel({
      ...
      repo: repoName,
      name: labelName,
      color: BOUNTY_LABEL_COLOR, // added here
      description: "Issue bounty label",
    });
  } catch (_) {}
}

async function getSponsoredCount(octokit, login) {
  try {
    const q = `
      query($login: String!) {
        user(login: $login) {
    ...
    `;
    const res = await octokit.graphql(q, { login });
    return res?.user?.sponsoring?.totalCount ?? 0;
  } catch (_) {
    return 0;
  }
}

async function upsertBountyBotComment(
  octokit,
  owner,
  repoName,
  issueNumber,
  ...
  const existing = comments.find((c) => {
    ...
    return isBot && (c?.body || "").includes(BOUNTY_COMMENT_MARKER); //added here
  });
  ...
  for (const e of timelineEvents) {
    ...
    try {
      ...
      const daysSinceUpdated = Math.floor(
        (new Date() - new Date(prData.updated_at)) / (1000 * 3600 * 24),
      );
...
async function ensureClosedPRLabel(octokit, owner, repoName) {
  try {
    ...
    if (e.status === 404) {
      ...
      console.log(
        `Failed to get label ${CLOSED_PR_LABEL} for ${owner}/${repoName}: ${e.message || e.stack}`,
      );
      throw e;
    }
  }
...
function extractLinkedIssuesFromPRBody(prBody, currentOwner, currentRepo) {
  if (!prBody) return [];
  const regex =
    /(?:close[sd]?|fix(?:e[sd])?|resolve[sd]?)(?:\s*:\s*|\s+)(?:#(\d+)|https?:\/\/github\.com\/([^\/]+)\/([^\/]+)\/issues\/(\d+))/gi;
...
async function findLinkedIssuesFromTimeline(
  octokit,
  owner,
  repoName,
  prNumber,
...
async function handleClosedPR(octokit, owner, repoName, pr, attribution) {
  ...
  const timelineIssues = await findLinkedIssuesFromTimeline(
    octokit,
    owner,
    repoName,
    pr.number,
...
async function handleOpenedPR(octokit, owner, repoName, pr, attribution) {
  ...
  for (const issueNumber of linkedIssues) {
    try {
      ...
      const warningComment = sortedComments.find((c) =>
        c.body?.includes(CLOSED_PR_COMMENT_MARKER),
      );
      if (warningComment) {
      ...
      console.log(
        `Issue #${issueNumber}: Removed label and warning, posted success comment`,
      );
    } catch (e) {
      console.log(`Failed to process issue #${issueNumber}: ${e.message}`);
    }
...
async function enforceClosedPRGracePeriod(
  octokit,
  owner,
  repoName,
  attribution,
  ...
  for (const issue of issues) {
    try {
      ...
      const linkedPRs = await getLinkedPRsWithDetails(
        octokit,
        owner,
        repoName,
        issue.number,
      ...
      const prNumber =
        linkedPRs.open.length > 0 ? linkedPRs.open[0].number : null;
      ...
      if (prAuthor) {
        ...
        if (isAssigned) {
          ...
          console.log(
            `PR author ${prAuthor} not assigned to issue, skipping unassignment`,
          );
        }
      } else {
        console.log(
          `Could not extract PR author from warning comment, skipping unassignment`,
        );
      }
...
const run = async () => {
  try {
    ...
    const bountyKeyword = "/bounty"; // added here

    if (
      (eventName === "issue_comment" && issue && comment) ||
      (eventName === "pull_request_review_comment" && pull_request && comment)
    ) {
      ...
      const shouldBounty = commentBody.startsWith(bountyKeyword); //Added here
            ...
            const unassignMessageExists = existingComments.data.some(
              (comment) =>
                comment.body.includes(
                  "⏰ This issue has been automatically unassigned from",
                ) ||
                comment.body.includes(
      ...
      if (shouldAssign) {
        try {
          ...
          if (linkedPRs.open.length > 0) {
            stalePRs = linkedPRs.open.filter(
              (pr) => pr.age >= STALE_PR_THRESHOLD_DAYS,
            );
            activePRs = linkedPRs.open.filter(
              (pr) => pr.age < STALE_PR_THRESHOLD_DAYS,
            );

            if (activePRs.length > 0) {
              // Block assignment - recent open PRs exist
              const prList = activePRs
                .map(
                  (pr) =>
          ...
          if (linkedPRs.closed.length > 0) {
            const closedList = linkedPRs.closed
              .map((pr) => {
          ...
          if (shouldSkipNormalAssignmentCheck) {
            ...
            try {
              ...
              console.error(
                `Failed to complete takeover assignment for issue #${issue.number}:`,
                error,
              );
              ...
              try {
                if (originalAssignees.length > 0) {
                  ...
                  await octokit.issues
                    ...
                    .catch(() =>
                      console.log("Label operation failed during restore"),
                    );
                ...
                console.error(
                  `Failed to restore original state for issue #${issue.number}:`,
                  restoreError,
                );
          ...
          if (currentAssignees.length > 0) {
            ...
            if (currentAssignee !== assigneeLogin) {
              console.log(
                `Issue #${issue.number} is already assigned to ${currentAssignee}`,
              );
            ...
            console.log(
              `Issue #${issue.number} is already assigned to ${assigneeLogin}. Notifying user.`,
            );
            await octokit.issues.createComment({
              owner,
              repo: repoName,
          ...
          try {
            ...
            console.error(
              `Failed to assign @${assigneeLogin} to issue #${issue.number}:`,
              error,
            );
            ...
            try {
              ...
              console.error(
                `Failed to clean up after assignment error for issue #${issue.number}:`,
                cleanupError,
              );
            }
          }
          ...
          console.error(
            `Error in assignment flow${issue ? ` for issue #${issue.number}` : ""}:`,
            error,
          );
        }
      } else if (shouldGiphy) {
        ...
        const giphyResponse = await axios.get(
          `https://api.giphy.com/v1/gifs/search?api_key=${giphyApiKey}&q=${searchText}&limit=1`,
        );
        ...
        if (kudosCommand.length >= 2) {
          ...
          console.log(
            `Sending kudos from ${sender} to @${receiver} with comment: "${kudosComment}"`,
          );
          ...
          try {
            ...
            console.log(
              `Note: Kudos not tracked on BLT (user may not have a profile): ${apiError.message}`,
            );
        ...
        if (tipCommand.length >= 3) {
          ...
          console.log(
            `Generating tip link from ${sender} to ${receiver} for $${amountValue}`,
          );
        ...
        if (!addAmount) {
          await octokit.issues.createComment({
            ...
            repo: repoName,
            issue_number: issue.number,
            body: `⚠️ Invalid /bounty format.\n\nUse: \`/bounty $amount\`\nExample: \`/bounty $5\`${attribution}`,
          });
          return;
        }
        ...
        const newLabel = `$${normalizedTotal} Bounty`;
        ...
        // sponsor count (best-effort)
        const sponsoredCount = await getSponsoredCount(
          octokit,
          comment.user.login,
        );
        ...
        const botBody =
          ...
          `This issue now has a total bounty of **$${normalizedTotal}** thanks to @${comment.user.login}.\n` +
          `They have sponsored **${sponsoredCount}** developers so far!\n\n` +
          `Want to contribute? Solve this issue and claim the reward.${attribution}`;

        await upsertBountyBotComment(
          octokit,
          owner,
          repoName,
          issue.number,
        ...
        console.log(`Bounty updated: ${newLabel}`);
      }
    }
        ...
        if (event.issue.assignee && event.issue.state === "open") {
          ...
          if (daysInactive > 1) {
            ...
            try {
              ...
              if (hasOpenPR) {
                console.log(
                  `Issue #${event.issue.number} has ${linkedPRs.open.length} open PR(s), checking if any are active...`,
                );

                // Check if any open PRs are less than 60 days old
                const activePRs = linkedPRs.open.filter(
                  (pr) => pr.age < STALE_PR_THRESHOLD_DAYS,
                );
                if (activePRs.length > 0) {
                  console.log(
                    `Issue #${event.issue.number} has active PR(s), skipping unassign.`,
                  );
                  continue;
                }
                ...
                console.log(
                  `All open PRs for issue #${event.issue.number} are stale (60+ days), proceeding with unassignment.`,
                );
              }
            } catch (searchError) {
              console.log(
                `Error checking for open pull requests for issue #${event.issue.number}:`,
                searchError,
              );
              console.log(
                `Skipping unassignment for issue #${event.issue.number} due to verification failure.`,
              );
              continue;
            }
