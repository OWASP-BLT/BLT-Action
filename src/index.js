      ...
      if (shouldUnassign) {
        ...
        try {
          ...
          if (hasAssignedLabel) {
            ...
            const unassignMessageExists = existingComments.data.some((comment) => comment.body.includes("⏰ This issue has been automatically unassigned from  due to 24 hours of inactivity.") || comment.body.includes("You have been unassigned from this issue."));
        ...
        try {
          ...
          for (const assignedIssue of assignedIssues) {
            ...
            try {
              ...
              console.error(`Failed to check PRs for issue #${assignedIssue.number}:`, searchError.message);
            }
          }
    ...
    try {
      ...
      for (const event of events) {
        if (event.issue.assignee && event.issue.state === "open") {
          ...
          if (daysInactive > 1) {
            console.log(`Checking issue #${event.issue.number} for inactivity`);
